...
import Database_GPS_Tracker  # import "Database_GPS_Tracker.py" file
...
class Processing:
    ...
    # ---------------------------Store raw data function starts---------------------------------
    def store_raw_date(self, foldername, raw_data):
        if len(raw_data) > 0:
            ...
            read_source_all = foldername + "/" + str(year) + "/" + str(month) + "/" + str(
                time_stamp.split(' ')[0].split('-')[2]) + ".txt"
    ...
    # ---------------------------|||Store raw data function stops|||---------------------------------

    # ----------------------------Lat Lon DB Update starts---------------------------------
    def latlondbupdate(equipment_id, latitude_dec, longitude_dec, time_stamp, speed, course_direction_dec, bat_power):
        ...
        try:
            ...
            if (cursor.rowcount > 0):
                ...
                if data != None:
                    ...
                    if (cursor.rowcount > 0):
                        ...
                        value3 = (
                            str(latitude_dec), str(longitude_dec), str(time_stamp), str(speed),
                            str(course_direction_dec),
                            str(bat_power), str(data['dev_id']))
            ...
            # ----------------------------Lat Lon DB Update stops---------------------------------

    # ----------------------------Fuel Percent DB Update starts---------------------------------
    def fueldbupdate(equipment_id, fuel_per):
        ...
        try:
            ...
            # ----------------------------Fuel Percent DB Update stops---------------------------------

    # ---------------------------Device Data Process Starts--------------------------------

    def device_data_process(self, client_data, equipment_id, client_address):
        ...
        try:
            ...
            if data_new[0:4] == "7878" and data_new[-4:].lower() == "0d0a" and data_new[6:8] == "12":
                ...
                self.latlondbupdate(equipment_id, latitude_dec, longitude_dec, time_stamp, speed, course_direction_dec,
                                    bat_power)
            ...
            if data_new[0:4] == "7878" and data_new[-4:].lower() == "0d0a" and data_new[6:8] == "16":
                ...
                from crccheck.crc import CrcX25
                data = bytearray.fromhex(data_new[4:-8])
                crc = hex(CrcX25.calc(data))
                error_chk = data_new[-8:-4]
