...
def usr_registration():
    ...
    try:
        ...
        # --- if requests list not null ---
        if usr_name_ != "" and usr_email_ != "" and usr_phone_ != "" and usr_address_ != "" and usr_password_ != "" and usr_account_type_ != "" and int(usr_account_type_) <= 2:
            ...
            if cursor.rowcount == 0:
                ...
                values1 = (usr_name_, usr_email_, "1", "1", usr_password_, usr_account_type_, app_id_, curr_time, curr_date,"")
    ...
    try:
        ...
        if usr_email_ != "" and usr_password_ != "":
            ...
            if user_mail_status:
                ...
                cursor.execute("SELECT usr_account_type, usr_email FROM user_profile WHERE usr_email=%s AND usr_password=%s", (usr_email_, usr_password_))
            else:
                # --- get valid the user form DB with name ---
                cursor.execute("SELECT usr_account_type, usr_email FROM user_profile WHERE usr_name=%s AND usr_password=%s", (usr_name, usr_password_))
            ...
            if acc_type is not None:
                ...
                if acc_type is not None:
                    ...
                    if user_mail_status:
                        ...
                        cursor.execute("UPDATE user_profile SET usr_fcmtoken = %s WHERE usr_email=%s", (usr_fcmtoken_, usr_email_))
                    else:
                        # --- update fcmToken with name ---
                        cursor.execute("UPDATE user_profile SET usr_fcmtoken = %s WHERE usr_name=%s", (usr_fcmtoken_, usr_email_))
                    conn.commit()
...
def devicedetails(dev_type=None, dev_id=None):
    ...
    if dev_id is None:
                ...
                if app_time != "":
                    ...
                    if cursor.rowcount > 0:
                        ...
                        if cursor.rowcount > 0:
                            ...
                            online_sql = """SELECT * FROM `device_data` WHERE TimeDiff(%s,updated_at) < '00:30:00' GROUP BY dev_id ORDER BY dev_id DESC"""
                            ...
                            if cursor.rowcount > 0:
                                ...
                                for dev_name in dev_names:
                                    ...
                                    dev_online_sql = """SELECT COUNT(*) FROM `devices_gateway` as dg 
                                                        JOIN `product_profile` as pp ON dg.dev_id = pp.dev_id AND pp.type_id = '%s' 
                                                        JOIN device_data as dd ON pp.dev_id = dd.dev_id WHERE TimeDiff(%s, dd.updated_at) < '00:30:00'"""
                                    ...
                                    dev_offline_sql = """SELECT COUNT(*) FROM `devices_gateway` as dg 
                                                            JOIN `product_profile` as pp ON dg.dev_id = pp.dev_id AND pp.type_id = '%s' 
                                                            JOIN device_data as dd ON pp.dev_id = dd.dev_id WHERE TimeDiff(%s, dd.updated_at) > '00:30:00'"""
                                    cursor.execute(dev_offline_sql, (dev_name[1], app_time))
                                    dev_offline_count = cursor.fetchone()[0]  # assign counted number in dev_offline_count
        ...
        # device details with the type parameter
        elif dev_type.lower() == "tractor" or dev_type.lower() == "porgen" or dev_type.lower() == "dieselgen" and dev_id is None:
            ...
            try:
                if app_time != "":
                    ...
                    cursor = conn.cursor()
                    # print(cursor)
                    product_sql = """SELECT pp.pro_name, pp.pro_id,up.usr_fname,up.usr_lname, dg.dev_s_n FROM product_profile as pp 
                                        LEFT JOIN assign_info as ai 
                                        ...
                                        LEFT JOIN product_type as pt ON pp.type_id=pt.type_id WHERE pt.type_name = '""" + dev_type + """'"""
                    cursor.execute(product_sql)
                    if cursor.rowcount > 0:
                        products = cursor.fetchall()
                        print(products)
        ...
        if dev_type.lower() == "tractor" or dev_type.lower() == "porgen" or dev_type.lower() == "dieselgen":
            ...
            try:
                if app_time != "":
                    ...
                    cursor = conn.cursor()
                    # print(cursor)
                    product_sql2 = """SELECT pp.pro_name, pp.pro_id,up.usr_fname,up.usr_lname, dg.dev_s_n FROM product_profile as pp 
                                        LEFT JOIN assign_info as ai ON pp.pro_id = ai.pro_id 
                                        LEFT JOIN user_profile as up ON ai.usr_id = up.usr_id
                                        LEFT JOIN devices_gateway as dg ON pp.dev_id = dg.dev_id
                                        LEFT JOIN product_type as pt ON pp.type_id=pt.type_id 
                                        WHERE pt.type_name = '""" + dev_type + """' AND pp.pro_id ='""" + id + """'"""
                    print(product_sql2)
    ...
    try:
        ...
        if pro_name_ != "" and dev_sn_ != "" and pmo_id_ != "" and ven_id_ != "" and type_id_ != "" and usr_id_ != "" and a_usr_id_ != "" and loc_lat_ != "" and loc_long_ != "" and loc_name_ != "":
            ...
            if cursor.rowcount == 0:
                ...
                # print(data["pmo_id"])
                # print(data["dev_id"])
                if data["pmo_id"] is None and data["dev_id"] is not None:
                    print("Not assigned")
                    ...
                    if cursor.rowcount > 0:
                        ...
                        return jsonify({"des": "Insert Failure", "err": "true"})
                elif data["pmo_id"] is not None and data["dev_id"] is not None:
...
def taskcount():
    try:
        ...
        if outofplan < 0:
            outofplan = 0
        if agent_id != "":
            data = {'total': total, 'completed': complete, 'inprogress': inprogress, 'outofplan': outofplan}
            # data['total'] = '15'
            if data['total'] == 0:
                data['done'] = 0
                data['todo'] = 0
